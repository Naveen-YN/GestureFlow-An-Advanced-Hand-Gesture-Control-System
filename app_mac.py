...
# Suppress TensorFlow oneDNN warnings (not typically an issue on macOS, but included for consistency)
...
try:
    ...
    SCREEN_WIDTH, SCREEN_HEIGHT = 1440, 900  # Fallback for typical MacBook resolution
        ...
        while True:
            try:
                ...
                if command == "Click":
                    ...
                    kb_controller.press(keyboard.Key.cmd)  # Use cmd instead of alt on macOS
                    ...
                    kb_controller.press(keyboard.Key.ctrl)
                    kb_controller.press('f')  # Cmd + Ctrl + F for fullscreen on macOS
                    kb_controller.release('f')
                    kb_controller.release(keyboard.Key.ctrl)
                    kb_controller.release(keyboard.Key.cmd)
                elif command == "Minimize Window":
                    kb_controller.press(keyboard.Key.cmd)
                    kb_controller.press('m')  # Cmd + M to minimize on macOS
        ...
        if finger_states == [0, 1, 0, 0, 0]:
            ...
            if pinch_dist < 50:
                ...
                if debounce:
                    ...
                    self.debounce_time[hand_id] = current_time
        elif finger_states == [0, 0, 1, 0, 0] and abs(dx) > 50:
...
def main():
    try:
        # Use default camera (0), adjust if necessary for external cameras
        cap = cv2.VideoCapture(0)
        if not cap.isOpened():
            raise RuntimeError("Failed to open webcam. Check camera permissions or index.")
